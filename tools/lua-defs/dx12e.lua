...
-- ★以下の 4 つはエディタのフライ操作専用。これを読むのは EngineMode::Editor の
--   経路だけなので、Play 中や配布したゲームでは呼んでも何も起きない。
--   ゲーム内のマウス感度・移動速度は自前で持つこと。
---@return number
...
-- ゲームモード起動時に自動適用される（窓の形を変える設定はエディタでは適用しない）。
-- ★VSync だけは窓の形を変えないので、エディタでも保存・復元される。
-- ============================================================
