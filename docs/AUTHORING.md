...
**シェーダーの仕分け（コンボが勝手にやること）**

Inspector のシェーダー選択コンボは、**用途の違うシェーダーを候補に出しません**。
頂点レイアウトが違うものは割り当てても必ず PSO 生成に失敗するので、
「選べたのに赤い枠が出る」より先に弾いています。判定は VS の入力シグネチャから
機械的に行うので、フォルダを移してもリネームしてもズレません。

| 用途 | 見分け方（自動） |
|---|---|
| メッシュ用 | 頂点入力に `POSITION` と `NORMAL` がある |
| スプライト用 | `POSITION` はあるが `NORMAL` が無い |
| 画面用 | 頂点入力が無い（`SV_VertexID` からフルスクリーン三角形を作る） |

さらに、コンボの中は**グループ見出しでまとまります**。グループ名の決まり方:

1. ソースのどこかに `// @group イベント` と書いてあればそれ
2. 無ければ**フォルダ名**（`assets/shaders/` は再帰スキャンなので `myfx/Glow.hlsl` の `myfx`）
3. どちらも無ければ「その他」

```hlsl
// @group イベント演出      ← この 1 行でコンボの見出しが決まる
```

用途で分けたいのは書いた人にしか分からないので、そこだけ申告制にしています。
何も書かなければフォルダで勝手に分かれるので、**まずはフォルダを切るだけでよい**です。
