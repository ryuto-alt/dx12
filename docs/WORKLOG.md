...
### 2026-07-31 — Hi-Z オクルージョンカリング
ブランチ `master`。`49dd57c` 〜 `eac5f5b`。既定 OFF（`settings.json` の
`"render_occlusion_culling"` / MCP `dx12_set_occlusion`）。

壁の裏に完全に隠れた描画を GPU 側で落とす。実装は
`src/renderer/HiZPass.{h,cpp}`（深度ピラミッド）+ `OcclusionCullPass.{h,cpp}`（可視性判定）+
`HiZMath.h`（純関数）+ `shaders/hiz/HiZBuild.hlsl` / `HiZCull.hlsl`。

**設計判断（憶測ではなく調べた上で決めた）**
- **2 フェーズ方式は使わない。** 2 フェーズは「遮蔽物専用パスを別途払いたくない」ための技法だが、
  このエンジンは深度プリパスが前方パスとビット厳密に一致する（同じ `m_drawItems` /
  同じジッタ付き `camVPJ` / 同じ LOD）ので既に払っている。プリパス後にピラミッドを建てれば
  同一フレーム・同一カメラの遮蔽情報になり、時間的なズレが原理的に発生しない
- **D3D12 のプレディケーションはクエリを必要としない**（「バッファ内の 64bit 値」で判定できる）。
  compute が書いた可視性バッファをそのまま述語にできるので、読み戻しゼロ・遅延ゼロで、
  ルートシグネチャ / マテリアル / PSO / シェーダを 1 行も触らずに済んだ。
  ルートシグネチャは既に 61/64 DWORD 使用済みで、`ExecuteIndirect` による GPU 駆動化は
  マテリアルのバインドレス化を伴う数週間仕事だったため現実的でなかった
- **読み戻し方式は却下**。3 フレーム遅延＝「速く振り向くと物が消える」で、ホラーゲームでは最悪

**踏んだ穴（同じ轍を踏まないための記録）**
- `w > 0` は近平面跨ぎの判定にならない。`w` はビュー空間 Z なので near の内側でも正の値で
  素通りする。跨ぎは透視除算後の NDC `z < 0` で捕まえる必要がある
- ミップ選択は `log2` の概算だけでは足りない。矩形が境界を跨ぐと概算ミップで 3 テクセルに
  広がる。整数テクセル座標で検算して 1 段上げないと、覆えていない領域の深度を根拠に
  遮蔽と誤判定して物が消える（`tests/hiz_math_test.cpp` が乱数 20000 件で不変条件を固定）
- 奇数サイズの取りこぼし。`floor` で縮約するので前段が奇数だと最後の 1 列/1 行がどの出力
  テクセルにも入らない。1052x592 のような非 2 冪では連鎖の途中で必ず奇数が出る
- **インスタンシングされた描画には述語をアイテム単位で張れない**。バッチは 1 ドローコール
  なので、1 体ぶんの判定で落とすと同じバッチの見えている物まで消える。バッチの合成 AABB を
  別途判定対象にした（実測: これが無いと city_blocks で述語が 1 本も張れなかった）

**正しさの検証（実測）**
- **決定的シーン（city_blocks）で深度プリパスを常時 ON に固定し、オクルージョンだけを
  切り替えた 15 構成（視点 5 x renderScale 3、非 2 冪の 789x444 / 526x296 含む）で、
  絵は全て 0 ピクセル差**。最大 868/3122 体（27.8%）をカリングし 478 ドローに述語を
  張った状態での結果＝見えている物を 1 ピクセルも消していない
- Nocturne（暗いシーン）20 構成 + Play 中 + Play/Stop 往復でも、A/B の差は対照実験
  （設定を変えず 2 枚撮る）のノイズ床以下で全て最大チャンネル差 1
- D3D12 デバッグレイヤ / GPU-Based Validation で新規メッセージ 0 件

**性能（benchmark 240 フレーム / uncapped）**
| シーン | プリパス | mainScene | GPU 合計 | drawCalls |
|---|---|---|---|---|
| Nocturne（TAA+SSAO ON） | 元々走る | 0.23→0.16 | 1.07→1.02 | 4656→4656 |
| city_blocks（プリパス無） | 強制で走る | 0.20→0.11 | 0.42→0.40 | 1068→**1543** |

★**ON にすると深度プリパスが強制的に走る**。元々走っているシーンなら追加コストは
`gpuPassMs.hiZ` の 0.04ms だけで `mainScene` が 3 割減るが、走っていないシーンで ON にすると
プリパスぶんの描画コールが増えて**遅くなる**（fps 604→549）。
★**どちらのテストシーンも GPU 律速ではない**（CPU バウンド）ので fps は改善しない。
効いてくるのは GPU 律速になってから。既定 OFF なのはこのため。

**未対応**
- 影パス。描画コールの約 90%（Nocturne で 3399/3758）がここだが、光源視点の Hi-Z が別途要る
- 部分的に隠れたインスタンシングのバッチ。バッチ全体が隠れているときしか落とせない。
  インスタンス単位で落とすには GPU 側でのインスタンス圧縮 + `ExecuteIndirect` が要る
