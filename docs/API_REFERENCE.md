...
（Scene Flow 窓の「演出を選ぶ / プレビュー…」からも開く）。
...
トランジション窓は**編集中のシーンではなく専用の架空ゲーム画面 2 枚**
（昼のフィールド → 夜のダンジョン）で再生する。遷移の中間点でその 2 枚が入れ替わるので、
幕の形と「隠れている間にシーンが差し替わる」様子の両方が確認できる。
再生 / 一時停止・ループ・再生速度（0.15〜2.0 倍）・コマ送りスクラブ付き。

| ID | 型番号 | 名前 | 既定秒 | 見た目 |
|---|---|---|---|---|
| `fade` | 0 | 暗転 | 1.0 | 画面が黒く沈んで戻る。どのジャンルでも外さない |
| `wipe` | 1 | 横ワイプ | 1.0 | 左から右へ拭き取る。軽快でテンポの速いゲーム向け |
| `iris` | 2 | アイリス | 1.0 | 四隅から円で閉じる。レトロ・アドベンチャーの定番 |
| `wipe_v` | 3 | 縦ワイプ | 1.0 | 上から下へ拭き取る。章立て・見出しの切り替えに |
| `seek` | 4 | シークバー早送り | 1.2 | 動画プレイヤー風。光るプレイヘッドが左→右へ2回掃き、画面下のシークバーが遷移全体の進行と同期して伸びる |
| `flash` | 5 | ホワイトアウト | 1.0 | 白く飛ばす。閃光・回想の入り・場面の飛躍に |
| `blinds` | 6 | ブラインド | 1.0 | 横帯が同時に閉じる。機械的・SF・ハッキング演出に |
| `clock` | 7 | 時計ワイプ | 1.2 | 12時から時計回りに扇が回る。時間・ターン制のモチーフに |
| `diamond` | 8 | 菱形 | 1.0 | アイリスの角ばった版。カード・パズル系の硬い印象に |
| `wipe_diag` | 9 | 斜めワイプ | 1.0 | 左上から右下へ斜めに拭く。スポーツ・レース系の勢いに |
| `curtain` | 10 | カーテン | 1.0 | 左右の幕が中央で合わさる。舞台・ステージ制・幕間に |
| `slide` | 11 | スライド | 1.0 | 一枚板が右から入り、そのまま左へ抜ける。メニュー・章送りに |
| `blinds_v` | 12 | 縦ブラインド | 1.0 | 縦帯が同時に閉じる。横スクロール・格ゲーの場面切替に |
| `star` | 13 | 星アイリス | 1.0 | 5 芒星が閉じる。ステージクリア・ごほうびの場面に |
| `plus` | 14 | 十字アイリス | 1.0 | 十字に閉じる。医療・回復・セーブポイントのモチーフに |
| `heart` | 15 | ハートアイリス | 1.0 | ハートが閉じる。恋愛・カジュアル・かわいい系に |
| `dissolve` | 16 | ディゾルブ | 1.0 | ノイズの粒がまばらに沈む。霧・砂・記憶が薄れる場面に |
| `mosaic` | 17 | モザイク | 1.0 | 四角いブロックがランダムに埋まる。レトロ・ドット絵の定番 |
| `melt` | 18 | メルト | 1.3 | 縦の列がばらばらの速さで垂れ落ちる。ホラー・レトロ FPS の融解 |
| `shatter` | 19 | ガラス割れ | 1.2 | 三角の破片が順に現れ、ひびが青白く走る。被弾・敗北・急転に |
| `glitch` | 20 | グリッチ | 1.0 | 走査帯がずれて色収差が出る。サイバー・電脳・システム異常に |
| `hex` | 21 | ハニカム | 1.1 | 六角形のセルが中心から広がる。SF・戦術・シールド展開の演出に |
| `checker` | 22 | チェッカー | 1.1 | 市松のマスが 2 段階で埋まる。ボード・パズル・レトロ UI に |
| `spiral` | 23 | 渦巻き | 1.2 | 中心から渦を巻いて飲み込む。ワープ・異空間への入り口に |
| `ripple` | 24 | 波紋 | 1.1 | 中心から波紋が広がり、縁が青く光る。水・魔法・詠唱の場面に |
| `flood` | 25 | 水没 | 1.2 | 下から水位が上がり、水面が波打つ。水中・沈没・潜行の入りに |
| `burn` | 26 | フィルムバーン | 1.2 | オレンジの焼け縁がフィルムを食う。回想・破棄・記録の終わりに |
| `rush` | 27 | 集中線 | 1.0 | 白い集中線が中心へ収束しながら閉じる。アニメ風の必殺技・戦闘突入に |

```lua
sceneTransition("scenes/level2.json")               -- エディタで選んだ演出
transitionToScene("scenes/boss.json", "shatter")    -- この呼び出しだけガラス割れ（1.2秒）
transitionToScene("scenes/boss.json", "iris", 1.5)  -- 秒も指定する
```

> **既定秒は 1.0 が下限の目安**。0.6 秒台だと「切り替わった」と気づく前に終わり、
> 星・渦・ガラス割れのような形のある演出はまったく読み取れない。
> 型番号（0〜27）は `settings.json` とシーン JSON に出る保存値なので値を動かさない
> （新しい型は必ず末尾へ足す）。Lua からは覚えなくていい ID 文字列を推奨。
> 未知の ID は警告を出して既定プリセットへ倒す。
>
> 演出の実体は `shaders/post/TransitionCurtain.hlsli` の `TransitionCurtain()` **ただ 1 箇所**。
> 実機（`Transition.hlsl`）もエディタのプレビュー窓とサムネイル（`TransitionPreview.hlsl`）も
> この同じ関数を呼ぶので、C++ 側に形の写しは無い＝「サムネイルと実物が違う」は起きない。
> プリセットの表は `src/renderer/TransitionPresets.h`、窓は
> `src/editor/panels/TransitionPreviewPanel.cpp`。型を足すときに触るのは
> 「`SceneTransition.h` の enum」「`TransitionCurtain.hlsli` の分岐」「`TransitionPresets.h` の表」の 3 つだけ。
