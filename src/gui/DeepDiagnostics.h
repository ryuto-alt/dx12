...
// Hi-Z オクルージョンカリングが「ON にしたのに効いていない」「ON にしたせいで遅くなっている」
// を名指しする検査。この機能は黙って損をする経路があるので、数字で言えるようにしてある:
//  - **ON なのにプリパスが他の誰にも要求されていない** … オクルージョンのためだけに深度
//    プリパスが走り、その描画コールぶん CPU が増える。GPU の節約より高くつくことがある
//    （実測: city_blocks で drawCalls 1068→1543 / fps 604→549）
//  - ON だが遮蔽率がほぼ 0 … 見晴らしの良いシーン/視点。判定コストだけ払っている
//  - ON だが述語を 1 本も張れていない … 全部インスタンシングのバッチで、かつバッチ全体が
//    隠れることが無い状態
//  - 正射 / 2D ビューで自動無効（enabled だが active でない）
DeepDiagReport HiZOcclusion(Application& app);
