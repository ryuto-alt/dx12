                                       ...
                                       const std::wstring& shaderDir)
{
    ...
    for (u32 f = 0; f < kFrameCount; ++f)
    {
        ...
        // [3..6] デカール枠（t18..t21）の仮埋め。
        // ★実体は DecalSystem::WriteSrvsInto が Application::Initialize の中で即座に上書きする
        //   （フォワード PS が t21 を Texture2D として宣言しているので、型の合わないまま
        //   描画に入らせない）。ここは「テーブルを Set した瞬間に未初期化が無い」ことの保険。
