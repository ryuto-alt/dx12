...
constexpr const char* kEngineVersion = "0.9.9";
...
constexpr const char* kWhatsNewTitle = "DX12 Engine v0.9.9 の更新内容";
constexpr const char* kWhatsNewBody =
    "今回の更新: Git連携パネルのバグ修正\n"
    "\n"
    "・修正: 日本語のコミットメッセージを入力すると文字が二重に書き込まれる不具合\n"
    "  (IME確定時の内部イベント処理の重複が原因)。\n"
    "・修正: 日本語のコミットメッセージがGitHubへ文字化けした状態でコミット/\n"
    "  プッシュされる、または失敗する不具合(内部エンコーディング変換の不備が原因)。\n";
} // namespace dx12e
