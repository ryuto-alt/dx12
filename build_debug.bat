...
powershell -NoProfile -Command "$psi = New-Object System.Diagnostics.ProcessStartInfo('cmake', '--build build/debug -j %DX12_BUILD_JOBS%'); $psi.UseShellExecute = $false; $p = [System.Diagnostics.Process]::Start($psi); try { $p.PriorityClass = 'BelowNormal' } catch {}; $p.WaitForExit(); exit $p.ExitCode" || exit /b 1
echo BUILD_DEBUG_OK
